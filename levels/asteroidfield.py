...
class AsteroidField(pygame.sprite.Sprite):
    ...
    def spawn(self, radius, position, velocity, space, kind):
        asteroid = Asteroid(position.x, position.y, radius, space, self.level, kind)
    ...
    def update(self, dt):
        ...
        if self.spawn_timer > self.asteroid_spawn_rate and self.level.current_asteroid_count <= self.max_asteroids:
            ...
            self.spawn(ASTEROID_MIN_RADIUS * kind, position, velocity, self.space, kind)
