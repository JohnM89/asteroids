...
class Spawner(pygame.sprite.Sprite):
    def __init__(self, level, entity_cls, scaling_factor, group_type, spawn_rate, current_count, max_count, max_radius, min_radius, height, width, kind, vectors=[(1,0),(-1,0),(0,1),(0,-1)]):
        super().__init__()
        ...
        def __spawn_wrapper(self, spawn_fn, *args):
            if isinstance(self.entity_cls, list):
                choice = random.choice(self.entity_cls)
            else:
                choice = self.entity_cls
            override = getattr(self.entity_cls, "spawn", None)
            if callable(override):
                return override(*args)
            return spawn_fn(choice, *args )
            
        
        def setup_spawn(self, radius, position, velocity):

            entity = self.entity_cls(position.x, position.y, radius, self.space, self.level)
        ...
        def spawn(self, *args):
            return self.spawn_wrapper(self.setup_spawn, self.entity_cls, *args)
        ...
        def update(self, dt):
            ...
            if self.spawn_timer > self.spawn_rate and self.current_count <= self.max_count:
                ...
                self.spawn(self.min_radius * kind, position, velocity)
